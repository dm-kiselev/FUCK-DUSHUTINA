...
int main( int argc, char **argv)
{
	...
	pthread_t proc_tid[5];
	for(int i = 0; i < 5; i++)
	{
		pthread_create(&proc_tid[i], NULL, &proc, NULL);
	}
	pthread_join(proc_tid[0], NULL);
...
void xtrFunction(struct data_calc_struct * data, struct data_processed *cdata)
{
	...
	if(data->step != 0)
	{
		printf("PROC: start calc %d\n", data->step);
		line.calcExtrapolationXY(data->x, data->y, AMOUNT_POINT, data->x_min, data->x_max, data->step, cdata->f);
	}
	if(data->step == 0)
	{
		printf("PROC: start calc %d\n", data->step);
		cdata->f[0] = line.calcExtrapolationXY(data->x, data->y, AMOUNT_POINT, data->x_min);
	}
}
