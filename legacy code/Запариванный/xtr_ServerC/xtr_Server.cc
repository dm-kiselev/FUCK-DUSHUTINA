	...
	while(1)
	{
		...
		switch(msg.type)
		{
			...
			case PROC_TRANSMISSION_DATA:
			{
				...
				if(info_client[index_proc]->clientId == -1)
					printf("SERVER: WARNING  clientId = -1 \n");
				break;
			}
			...
			case PROC_READY:
			{
				...
				if(fPC != -1 )
				{
					//int index_unk = searchProcForID(rcvid, info_client);
					//if(index_unk != -1)
					//	break;
					
					
					flagSendMsg = false;
					printf("SERVER: *****type %d\n", info_client[0]->clientId );
					...
					printf("WARNING: SEND TO PROC %d %d\n",procID[index_free][0],
...
int searchProcForID(int id, _struct_client_id ** inf_client)
{
	printf("SERVER: Client ID: ");
	for(int j = 0; j < AMOUNT_PROC; j++)
	{
		printf("%d ", inf_client[j]->clientId );
	}
	printf("\n");
